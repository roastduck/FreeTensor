...
template<int n>
std::array<std::pair<std::string, int>, n> split_loop(Schedule &schedule, std::string loop, std::array<int, n> tiling) {
...
SketchPart MultiLevelTilingPart::mutate() {
    ...
    if (mut_part == 0) {
        mut.annotation.i_tiling = random_fill_array<4>(target.i.end - target.i.begin);
    } else if (mut_part == 1) {
        mut.annotation.j_tiling = random_fill_array<4>(target.j.end - target.j.begin);
    } else {
        mut.annotation.k_tiling = random_fill_array<2>(target.k.end - target.k.begin);
    }

...
def test_shorten_for(as_subprocess):
...
@pytest.mark.parametrize("as_subprocess", [False, True])
def test_side_effect_intrinsic(as_subprocess):
    with ft.For("i", 0, 10) as i:
        with ft.If(i < 4):
...
@pytest.mark.parametrize("as_subprocess", [False, True])
def test_multiple_iterators(as_subprocess):
...
@pytest.mark.parametrize("as_subprocess", [False, True])
def test_linear_bounds(as_subprocess):
...
@pytest.mark.parametrize("as_subprocess", [False, True])
def test_presburger_bounds(as_subprocess):
...
@pytest.mark.parametrize("as_subprocess", [False, True])
def test_presburger_bounds_strided(as_subprocess):
    with ft.VarDef([("x", (128, 128), "int32", "input", "cpu"),
                    ("y", (128, 128), "int32", "output", "cpu")]) as (x, y):
        with ft.For("i0", 0, 8) as i0:
            with ft.For("j0", 0, 8) as j0:
...
@pytest.mark.parametrize("as_subprocess", [False, True])
def test_multiple_branches(as_subprocess):
    ...
    ast1 = ft.shrink_for(ast1, as_subprocess=as_subprocess)
    print(ast1)
    ...
    ast2 = ft.shrink_for(ast2, as_subprocess=as_subprocess)
    print(ast2)
